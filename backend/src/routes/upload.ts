...
function runMulter(req: Request, res: import("express").Response): Promise<void> {
...
router.post("/", requireAuth, uploadLimiter, async (req, res) => {
  ...
  try {
    ({ publicFile, privateFile } = await processImage(
      imageFile.buffer,
      imageFile.originalname,
    ));
  } catch (err) {
    res.status(422).json({ error: `Failed to process image: ${(err as Error).message}` });
    return;
  }
